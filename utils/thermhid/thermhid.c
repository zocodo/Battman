...
void set_interval(IOHIDServiceClientRef service, int interval) {
    ...
    IOHIDServiceClientSetProperty(service, CFSTR(kIOHIDReportIntervalKey), number);
    ...
    if (IOHIDEventGetType(event) == kIOHIDEventTypeTemperature) {
        ...
        if (CFDictionaryContainsKey((CFDictionaryRef)refcon, number)) {
            ...
            if (dict) {
                ...
                CFStringRef Product;
                do {
                    if (count < 2 || CFDictionaryGetValue(dict, CFSTR(kIOHIDProductKey))) {
                        LocationID = CFDictionaryGetValue(dict, CFSTR(kIOHIDLocationIDKey));
                        Product = CFDictionaryGetValue(dict, CFSTR(kIOHIDProductKey));
...
repeat:
    ...
    int buf;
    CFIndex count;
    void *keys[3] = {(void *)CFSTR(kIOHIDPrimaryUsagePageKey), (void *)CFSTR(kIOHIDPrimaryUsageKey), (void *)CFSTR(kIOHIDLocationIDKey)};
    void *values[3];
    ...
    if (CFArrayGetCount(services) >= 1) {
        ...
        do {
            ...
            LocationID = IOHIDServiceClientCopyProperty((IOHIDServiceClientRef)ptr, CFSTR(kIOHIDLocationIDKey));
            Product = IOHIDServiceClientCopyProperty((IOHIDServiceClientRef)ptr, CFSTR(kIOHIDProductKey));

            format_print(LocationID, Product);

            IOHIDEventRef event = IOHIDServiceClientCopyEvent((IOHIDServiceClientRef)ptr, selected_grp->event, 0, 0);
            if (event) {
                printf("%g", IOHIDEventGetFloatValue(event, selected_grp->field));
                CFRelease(event);
            } else {
                printf("--");
                ...
                if (do_callbacks) {
                    ...
                    if (callback_addr) {
                        ...
                        if (dict) {
                            if (Product) CFDictionarySetValue(dict, CFSTR(kIOHIDProductKey), Product);
                            if (LocationID) CFDictionarySetValue(dict, CFSTR(kIOHIDLocationIDKey), LocationID);
                        }
